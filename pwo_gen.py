...
os.makedirs("img_pwo", exist_ok=True)
os.makedirs("netcdf", exist_ok=True)

date = input("Enter model init time (Use YYYY-MM-DD-HH format!): ")

run = pd.Timestamp(date)
...
filename = f'nam_{date}.nc'
ds.to_netcdf(f"netcdf/{filename}")
...
for value in hours:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('img_pwo', f"pwo_{date}_{ts:02d}.png"))
    plt.show()
